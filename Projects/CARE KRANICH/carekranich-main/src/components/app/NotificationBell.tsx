        ...
        <div className="absolute right-0 top-12 z-40 w-80 rounded-2xl border border-border bg-card p-2 shadow-elevated">
          ...
          <div className="max-h-96 overflow-y-auto">
            ...
            {notifs.map((n) => {
              const inner = (
                <div
                  className={`rounded-xl px-3 py-2 ${n.read_at ? "" : "bg-cream/60"}`}
                >
                  <div className="flex items-start justify-between gap-2">
                    <p className="text-sm text-foreground">{n.title}</p>
                    {!n.read_at && <span className="mt-1.5 h-1.5 w-1.5 flex-none rounded-full bg-wine" />}
                  </div>
              ...
              return n.link ? (
                <Link key={n.id} to={n.link} className="block hover:bg-cream/40" onClick={() => setOpen(false)}>
                  {inner}
                </Link>
              ) : (
