...
function ExamDetail() {
  ...
  if (exam.isLoading) {
    return (
      <MarketingPage title="Carregando exame..." crumbs={[{ label: "Exames", to: "/exames" }]}>
        <p className="text-sm text-muted-foreground">Buscando informações do exame.</p>
      </MarketingPage>
    );
  }
  ...
  if (!exam.data) {
    return (
      <MarketingPage
        ...
        primaryCta={{ label: "Ver catálogo completo", to: "/exames" }}
      >
        <p className="text-sm text-muted-foreground">
          Este exame pode ter sido removido do catálogo público. Explore o catálogo completo ou fale conosco pelo chat.
        </p>
      </MarketingPage>
    );
  }
    ...
    >
      <div className="grid gap-6 md:grid-cols-3">
        <div className="rounded-2xl border border-border bg-card p-6 shadow-soft">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {e.allow_installments ? "Parcelamento disponível" : "Pagamento à vista"}
            {e.insurance_accepted ? ` · Convênios: ${e.insurance_accepted}` : ""}
          </p>
        </div>
        <div className="rounded-2xl border border-border bg-card p-6 shadow-soft">
          <p className="text-xs uppercase tracking-widest text-moss">Prazo do resultado</p>
          <p className="mt-2 font-display text-3xl text-foreground">
            {e.turnaround_days ? `${e.turnaround_days} dias` : "Consulte"}
          </p>
          <p className="mt-1 text-xs text-muted-foreground">
            {e.home_collection ? "Coleta domiciliar disponível" : "Coleta em unidade"}
          </p>
        </div>
        <div className="rounded-2xl border border-border bg-card p-6 shadow-soft">
          <p className="text-xs uppercase tracking-widest text-moss">Preparo</p>
          <p className="mt-2 text-sm leading-relaxed text-foreground">
            {e.preparation || "Nenhum preparo especial"}
          </p>
          <p className="mt-1 text-xs text-muted-foreground">
            {e.fasting_hours ? `Jejum de ${e.fasting_hours} horas` : "Não exige jejum"}
          </p>
        </div>
      </div>

      <Section title="Sobre este exame" kicker="Informações clínicas">
        <div className="grid gap-4 md:grid-cols-2">
          {e.benefits && <InfoCard label="Benefícios" value={e.benefits} />}
          {e.indication && <InfoCard label="Para quem é recomendado" value={e.indication} />}
          {e.contraindication && <InfoCard label="Para quem não é indicado" value={e.contraindication} />}
          {e.biological_material && <InfoCard label="Material coletado" value={e.biological_material} />}
          {e.collection_method && <InfoCard label="Método de coleta" value={e.collection_method} />}
          {e.technology && <InfoCard label="Tecnologia utilizada" value={e.technology} />}
          {e.min_age || e.max_age ? (
            <InfoCard
              label="Faixa etária"
              value={`${e.min_age ? `A partir de ${e.min_age} anos` : "Sem idade mínima"}${e.max_age ? ` · até ${e.max_age} anos` : ""}`}
            />
          ) : null}
          {e.risks && <InfoCard label="Riscos" value={e.risks} />}
          {e.limitations && <InfoCard label="Limitações" value={e.limitations} />}
        </div>
      </Section>

      {isGenetic && (
        <Section title="Informações genéticas importantes" kicker="Genética responsável">
          <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="mt-4 rounded-2xl border border-terracotta/30 bg-terracotta/5 p-6 text-sm leading-relaxed text-muted-foreground">
            ...
            {e.requires_counseling && (
              <p className="mt-1">Recomendamos aconselhamento genético antes e após o resultado.</p>
            )}
            <p className="mt-1">
              Predisposição genética não significa diagnóstico. Os resultados têm acesso restrito, protegido por
              consentimento explícito e trilha de auditoria, e devem ser interpretados por profissionais habilitados.
            </p>
          </div>
        </Section>
      )}
      ...
      {e.requires_screening && (
        <div className="mt-10 rounded-2xl border border-border bg-ivory/60 p-6 text-sm text-muted-foreground backdrop-blur">
          Este exame passa por uma triagem rápida antes do agendamento para garantir sua segurança.
        </div>
      )}

      {Array.isArray(e.faq) && e.faq.length > 0 && (
        <Section title="Perguntas frequentes" kicker="Tire suas dúvidas">
      ...
      {(related.data ?? []).length > 0 && (
        <Section title="Exames relacionados" kicker="Você também pode precisar">
