...
function Tenants() {
  ...
  const orgReport = async (tenant: any) => {
    ...
    downloadPdf(
      ...
      `Relatório da organização - ${tenant.name}`,
      [
        line("Nome", tenant.name),
        line("Razão social", tenant.legal_name),
        line("CNPJ", tenant.cnpj),
        line("E-mail", tenant.email),
        line("Telefone", tenant.phone),
        line("Responsável", tenant.responsible_name),
        line("Endereço", tenant.address),
        line("Cidade/UF", [tenant.city, tenant.state].filter(Boolean).join(" / ")),
        line("CEP", tenant.postal_code),
        line("País", tenant.country),
        "",
        line("Status", tenant.status),
        line("Cobrança", tenant.billing_status),
        line("Arquivada", tenant.archived_at ? new Date(tenant.archived_at).toLocaleDateString("pt-BR") : "Não"),
        line("Código de convite", tenant.invite_code),
