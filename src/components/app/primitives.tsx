...
export function Card({
  ...
  return (
    <div
      className={`rounded-2xl border border-white/70 bg-white/60 shadow-soft ring-1 ring-white/35 backdrop-blur-2xl ${padded ? "p-6" : ""} ${className}`}
    >
      {children}
    </div>
  );
...
export function SectionHeader({
  ...
  return (
    <div className="mb-6 flex items-end justify-between gap-4">
      <div>
        {eyebrow && <p className="text-xs font-semibold uppercase text-moss">{eyebrow}</p>}
        <h2 className="mt-1 text-2xl font-semibold text-foreground md:text-3xl">{title}</h2>
      </div>
      {action}
    </div>
  );
...
export function Stat({
  ...
  return (
    <Card>
      ...
      <p className={`mt-2 text-3xl font-semibold tabular-nums ${toneMap[tone]}`}>{value}</p>
...
export function Pill({
  ...
  children: ReactNode;
  tone?: "moss" | "wine" | "gold" | "olive" | "terracotta" | "muted";
}) {
  const toneMap: Record<string, string> = {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium ${toneMap[tone]}`}
    >
      {children}
    </span>
  );
...
export function Avatar({
  ...
  return (
    <div
      className={`flex flex-none items-center justify-center rounded-full font-semibold ${tones[tone] || tones.olive}`}
