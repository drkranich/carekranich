...
const workflows = [
  {
    name: "Missed medication to Family escalation",
    cat: "Medication",
    runs: 1248,
    success: 99.2,
    steps: [
  ...
  {
    name: "Abnormal vitals to Telemedicine bridge",
    cat: "Health",
    runs: 412,
    success: 98.5,
    steps: [
      "Detect anomaly (2 readings)",
      "Page caregiver",
      "Open telemed slot",
      "Doctor joins",
      "Plan updated",
  ...
  {
    name: "Fall detected to Full emergency chain",
    cat: "Emergency",
    runs: 18,
    success: 100,
    steps: [
  ...
  {
    name: "Emotional withdrawal to Companion intervention",
    cat: "Emotional",
    runs: 86,
    success: 94.1,
    steps: [
      "Detect 3-day pattern",
      "Care Kranich companion call",
      "Notify family gently",
      "Suggest visit",
      "Wellness coach",
    ],
  },
  ...
  return (
    <>
      <PageHeader
        ...
        action={
          <button className="rounded-full bg-olive px-4 py-2 text-sm text-ivory shadow-soft">
            + New workflow
          </button>
        }
      />
      ...
      <div className="grid gap-6 lg:grid-cols-2">
          ...
          <Card key={w.name}>
            <div className="flex items-start justify-between">
              <div>
                ...
                <h3 className="mt-1 text-lg font-semibold text-foreground">{w.name}</h3>
              </div>
                ...
                {w.steps.map((s, i) => (
                  <div key={i} className="flex items-center gap-2">
                    <div className="rounded-2xl border border-border bg-cream/40 px-3 py-2 text-xs text-foreground whitespace-nowrap">
                      <span className="mr-2 inline-flex h-5 w-5 items-center justify-center rounded-full bg-olive font-display text-[10px] text-ivory">
                        {i + 1}
                      </span>
                      {s}
                    </div>
        ...
        <Card className="lg:col-span-2 bg-gradient-olive text-ivory border-none">
          <p className="text-xs uppercase text-ivory/70">AI suggestion</p>
          <h3 className="mt-1 text-2xl font-semibold">
            Create a workflow for nighttime restlessness?
          </h3>
          <p className="mt-2 max-w-2xl text-sm text-ivory/85">
            Care Kranich detected 7 nights of light fragmentation patterns across 12 residents. A
