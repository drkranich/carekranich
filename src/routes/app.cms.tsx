...
function Cms() {
  ...
  const createPage = useMutation({
    mutationFn: async () => {
      if (newPage.title.trim().length < 2) throw new Error("Enter the page title.");
    ...
    onError: (e: any) => toast.error(e.message ?? "Could not create the page"),
  });
  ...
  const togglePublish = async () => {
    ...
    toast.success(selected.published ? "Page unpublished" : "Page published");
    refresh();
  };

  const deletePage = async () => {
    if (!selected) return;
    if (!window.confirm(`Delete page "${selected.title}"?`)) return;
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Edit public pages by blocks - text, highlights, callouts and FAQ - without touching code."
        action={
          selected && (
      ...
      <Card className="space-y-3 p-6">
        <h3 className="flex items-center gap-2 text-sm font-semibold text-foreground">
          <Globe className="h-4 w-4" /> New public page
        </h3>
        <div className="grid gap-3 md:grid-cols-3">
          <input className={glassInput} placeholder="Title (e.g. About Care Kranich)" value={newPage.title} onChange={(e) => setNewPage({ ...newPage, title: e.target.value })} />
          <input className={glassInput} placeholder="Address (optional, e.g. about)" value={newPage.slug} onChange={(e) => setNewPage({ ...newPage, slug: e.target.value })} />
      ...
      <div className="grid gap-6 xl:grid-cols-[300px_1fr]">
        <Card className="space-y-2 p-5">
          ...
          {(pages.data ?? []).length === 0 && <p className="text-sm text-muted-foreground">No pages yet.</p>}
        ...
        {selected && current ? (
          <Card className="space-y-4 p-6">
            <div className="grid gap-3 md:grid-cols-3">
              <input className={glassInput} placeholder="Eyebrow above title" value={current.hero_eyebrow} onChange={(e) => setDraft({ ...current, hero_eyebrow: e.target.value })} />
              <input className={`${glassInput} md:col-span-2`} placeholder="Main title" value={current.title} onChange={(e) => setDraft({ ...current, title: e.target.value })} />
              <textarea className={`${glassInput} md:col-span-3`} rows={2} placeholder="Subtitle / opening callout" value={current.subtitle} onChange={(e) => setDraft({ ...current, subtitle: e.target.value })} />
            </div>
            ...
            <div className="space-y-3">
              {current.sections.length === 0 && (
                <p className="text-xs text-muted-foreground">No blocks yet. Add blocks above and drag to reorder.</p>
              )}
              {current.sections.map((s, index) => (
                ...
                >
                  ...
                  <input className={glassInput} placeholder="Block title" value={s.title ?? ""} onChange={(e) => patchSection(s.id, { title: e.target.value })} />

                  {(s.type === "texto" || s.type === "chamada") && (
                    <textarea className={glassInput} rows={3} placeholder="Text (each line becomes a paragraph)" value={s.body ?? ""} onChange={(e) => patchSection(s.id, { body: e.target.value })} />
                  )}

                  {s.type === "chamada" && (
                    <div className="grid gap-2 md:grid-cols-2">
                      <input className={glassInput} placeholder="Button text" value={s.cta_label ?? ""} onChange={(e) => patchSection(s.id, { cta_label: e.target.value })} />
                      <input className={glassInput} placeholder="Button link (e.g. /signup)" value={s.cta_url ?? ""} onChange={(e) => patchSection(s.id, { cta_url: e.target.value })} />
                    </div>
                  )}
                    ...
                    <div className="space-y-2">
                      {(s.items ?? []).map((item, i) => (
                        <div key={i} className="grid gap-2 md:grid-cols-[1fr_2fr_auto]">
                          <input className={glassInput} placeholder={s.type === "faq" ? "Question" : "Card title"} value={item.title} onChange={(e) => patchSection(s.id, { items: (s.items ?? []).map((x, j) => (j === i ? { ...x, title: e.target.value } : x)) })} />
                          <input className={glassInput} placeholder={s.type === "faq" ? "Answer" : "Description"} value={item.body} onChange={(e) => patchSection(s.id, { items: (s.items ?? []).map((x, j) => (j === i ? { ...x, body: e.target.value } : x)) })} />
            ...
            <div className="flex gap-2">
              <button onClick={() => savePage.mutate()} disabled={savePage.isPending || !draft} className="rounded-full bg-olive px-5 py-2 text-sm font-medium text-ivory shadow-soft hover:opacity-90 disabled:opacity-50">
                {savePage.isPending ? "Saving..." : draft ? "Save changes" : "No changes"}
              </button>
              {draft && (
          ...
          <EmptyState title="No page selected" hint="Create the first public page and build its content by blocks." />
        )}
      </div>
    </div>
  );
