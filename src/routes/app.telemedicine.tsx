...
function Telemedicine() {
  ...
  const [subject, setSubject] = useState("Telemedicine consultation");
  ...
  return (
    <>
      <PageHeader
        title="Telemedicine"
        subtitle="Real requests and clinical events, with an instant video room for each consultation."
        action={<Pill tone={telemed.isError ? "wine" : "olive"}>{telemed.isError ? "Read error" : "Video integrated"}</Pill>}
      />
        ...
        <div className="grid gap-6 lg:grid-cols-[.8fr_1.2fr]">
          <Card>
            ...
            <div className="mt-4 space-y-3">
              ...
              <button onClick={createRequest} disabled={!profile?.tenant_id} className="w-full rounded-xl bg-olive px-4 py-2 text-sm text-ivory disabled:opacity-50">Create request</button>
            </div>
            <p className="mt-4 text-xs text-muted-foreground">{telemed.data?.doctors.length ?? 0} doctors visible on the platform.</p>
            <div className="mt-4 rounded-2xl border border-baby/45 bg-baby/18 p-4">
              <Pill tone="gold">Google Meet planned</Pill>
              <p className="mt-3 text-sm leading-6 text-muted-foreground">
                Current rooms use instant secure video (Jitsi) without requiring an account. Automatic Google Meet
                link creation will be connected when Google API credentials are configured.
              </p>
            </div>
          </Card>
          ...
          <Card>
            ...
            {(telemed.data?.threads ?? []).length === 0 ? (
              ...
              <div className="mt-5 space-y-3">
                ...
                <Pill tone={thread.status === "closed" ? "muted" : "olive"}>{thread.status === "closed" ? "closed" : "open"}</Pill>
                        <button
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">{new Date(thread.last_message_at).toLocaleString("en-US")}</p>
                  </div>
                ))}
              </div>
            )}
