    ...
    <>
      ...
      <Card className="mb-6">
        ...
        <p className="mt-2 text-sm text-foreground">
          Agents share a tenant-scoped memory layer, a provider-agnostic model gateway, and a
          recommendations bus. Each one is role-gated and resident-aware where relevant. Switching
          AI providers does not require UI changes.
        </p>
        <div className="mt-4 grid gap-3 sm:grid-cols-4 text-xs">
          {[
            "Resident & Twin context",
            "Tenant memory",
            "Provider gateway",
            "Recommendations bus",
          ].map((p) => (
            <div
              key={p}
              className="rounded-2xl border border-border bg-cream/40 px-3 py-2 text-center"
            >
              {p}
            </div>
          ))}
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {visible.map((a) => {
          ...
          return (
            <Link
              key={a.key}
              to="/app/agents/$agentKey"
              params={{ agentKey: a.key }}
              className="group"
            >
              <Card className="h-full transition-all group-hover:shadow-elevated">
                <div className="flex items-start gap-3">
                  <div
                    className={`flex h-11 w-11 items-center justify-center rounded-2xl bg-${a.tone}/10 text-${a.tone}`}
                  >
                    <svg
                      viewBox="0 0 24 24"
                      className="h-5 w-5"
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {a.capabilities.map((cap) => (
                    <span
                      key={cap}
                      className="rounded-full bg-cream px-2 py-0.5 text-[11px] text-foreground/70"
                    >
                      {cap}
                    </span>
                  ))}
