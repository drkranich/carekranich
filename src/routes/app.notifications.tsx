        ...
        <Card className="lg:col-span-2 p-0">
          <div className="border-b border-border/60 px-6 py-4 flex items-center justify-between">
            ...
            <div className="flex gap-2 text-xs">
              ...
              <button className="rounded-full border border-border px-3 py-1 text-muted-foreground">Unread</button>
              <button className="rounded-full border border-border px-3 py-1 text-muted-foreground">Mentions</button>
            </div>
          </div>
          {groups.map((g) => (
            <div key={g.label}>
              <p className="px-6 pt-4 pb-2 text-[10px] uppercase tracking-widest text-muted-foreground">{g.label}</p>
              <ul>
                {g.items.map((n, i) => (
                  <li key={i} className="flex items-start gap-4 px-6 py-3 hover:bg-cream/40 border-b border-border/60 last:border-b-0">
            ...
            <ul className="mt-3 space-y-3 text-sm">
              {[
                ...
                <li key={c.c} className="flex items-center justify-between rounded-xl border border-border/60 bg-cream/40 p-3">
                  ...
                  <span className={`relative inline-flex h-5 w-9 items-center rounded-full transition ${c.on ? "bg-olive" : "bg-border"}`}>
                    <span className={`inline-block h-4 w-4 transform rounded-full bg-ivory transition ${c.on ? "translate-x-4" : "translate-x-0.5"}`} />
                  </span>
                </li>
              ))}
            </ul>
