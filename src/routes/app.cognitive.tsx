...
const SOURCE_OPTIONS = [
  ...
  { value: "nurse", label: "Nursing observation" },
  { value: "doctor", label: "Medical evaluation" },
  { value: "assessment", label: "Standardized assessment" },
  { value: "ai", label: "IA" },
];
    ...
    return (
      <>
        <PageHeader
          ...
          subtitle="Memory, attention, language, reasoning, executive function, emotional stability."
        />
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={
          selected
            ? `${selected.preferred_name || selected.full_name} - ${assessments.length} assessments`
            : ""
        }
        action={
      ...
      {showForm && canAssess && (
        <AssessmentForm
          ...
          onSubmit={(v) => addAssessment.mutate(v)}
          submitting={addAssessment.isPending}
        />
      )}

      <div className="grid gap-6 lg:grid-cols-3">
        <Card>
          <p className="text-xs uppercase text-muted-foreground">Cognitive vitality</p>
          <div className="mt-4">
            <Ring
              value={vitality}
              ...
              sub={
                latest
                  ? `as of ${new Date(latest.assessed_at).toLocaleDateString()}`
                  : "no assessments"
              }
              color="var(--olive)"
              size={160}
        ...
        <Card className="lg:col-span-2">
          <p className="text-xs uppercase text-muted-foreground">Dimensions</p>
              ...
              return (
                <div key={d.key} className="rounded-2xl border border-border bg-cream/40 p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`text-xs ${delta > 0 ? "text-moss" : delta < 0 ? "text-wine" : "text-muted-foreground"}`}
                    >
                      {delta > 0 ? "up" : delta < 0 ? "down" : "-"} {Math.abs(delta)}
                    </span>
                  </div>
                  <div className="mt-2 flex items-end justify-between">
                    <span className="text-3xl font-semibold text-olive">{score ?? "-"}</span>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        <Card>
          <div className="flex items-center justify-between">
            <p className="text-xs uppercase text-muted-foreground">Cognitive timeline</p>
            <Pill tone="moss">live</Pill>
          </div>
          {timeline.length === 0 ? (
            <p className="mt-4 text-sm text-muted-foreground">
              No cognitive events recorded yet. Memory events, confusion episodes, exercises, and
        ...
        <Card>
          <p className="text-xs uppercase text-muted-foreground">AI cognitive insights</p>
          {insights.length === 0 ? (
            <p className="mt-4 text-sm text-muted-foreground">
              Insights will appear once enough assessments and observations are recorded.
            </p>
          ) : (
        ...
        {DIMENSIONS.map((d) => (
          <Field key={d.key} label={`${d.label} (0-100)`}>
            <input
              ...
              max="100"
              value={state[d.key] ?? ""}
              onChange={(e) => upd(d.key, e.target.value)}
            />
          </Field>
        ))}
        <Field label="Source">
        ...
        <div className="md:col-span-3">
          <Field label="Notes">
            <textarea
              ...
              placeholder="Observations, context, conditions during assessment..."
            />
          </Field>
        </div>
