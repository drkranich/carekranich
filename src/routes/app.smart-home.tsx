...
const rooms = [
  { name: "Bedroom", x: 0, y: 0, w: 2, h: 2, status: "Resting · 22°C", tone: "moss", active: false },
  { name: "Bathroom", x: 2, y: 0, w: 1, h: 2, status: "Quiet", tone: "muted", active: false },
  { name: "Living room", x: 0, y: 2, w: 2, h: 2, status: "Maria here · reading", tone: "wine", active: true },
  ...
  { name: "Hall", x: 3, y: 0, w: 1, h: 2, status: "Front door locked", tone: "moss", active: false },
];

function SmartHome() {
  return (
    <>
      <PageHeader title="Home guardianship" subtitle="Maria's home · 6 rooms · 14 sensors · 2 wearables" action={<Pill tone="moss">Safety score 96/100</Pill>} />

      <div className="grid gap-4 md:grid-cols-4">
        <Stat label="Movement" value="Normal" sub="Last: 3 min ago" tone="moss" />
        <Stat label="Air quality" value="Excellent" sub="CO₂ 480 ppm" tone="moss" />
        <Stat label="Temperature" value="22.4°C" sub="Stable" tone="gold" />
        <Stat label="Door status" value="Secure" sub="Locked since 21:14" tone="olive" />
      </div>

      <div className="mt-6 grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <p className="text-xs uppercase tracking-widest text-muted-foreground">Live floor plan</p>
          <div className="mt-4 grid grid-cols-4 grid-rows-4 gap-2 rounded-2xl bg-cream/40 p-4" style={{ aspectRatio: "4/3.5" }}>
            {rooms.map((r) => (
              <div
                key={r.name}
        ...
        <Card>
          <p className="text-xs uppercase tracking-widest text-muted-foreground">Connected devices</p>
          <ul className="mt-4 space-y-3">
            {[
              ...
              <li key={d.d} className="flex items-center justify-between rounded-2xl border border-border/60 bg-cream/40 p-3">
                <div>
        ...
        <Card className="lg:col-span-3">
          <p className="text-xs uppercase tracking-widest text-muted-foreground">Behavioral pattern · last 7 nights</p>
          <h3 className="mt-1 font-display text-xl text-foreground">Nighttime stability — improving</h3>
          <div className="mt-6 grid grid-cols-7 gap-2">
            {[
              ...
              <div key={n.d} className="rounded-2xl border border-border bg-cream/40 p-4 text-center">
                <p className="text-xs text-muted-foreground">{n.d}</p>
                <p className="mt-2 font-display text-2xl text-olive">{n.e}</p>
