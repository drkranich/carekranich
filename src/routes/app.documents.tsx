  ...
  return (
    <>
      <PageHeader
        ...
        action={
          <button className="rounded-full bg-olive px-4 py-2 text-sm text-ivory">Upload</button>
        }
      />
      ...
      <Card className="mb-6">
        ...
        <div className="mt-3 flex flex-wrap gap-2 text-xs">
          {["All", "Medical", "Prescription", "Contract", "Insurance", "Certification"].map(
            (t, i) => (
              <button
                key={t}
                className={`rounded-full px-3 py-1 ${i === 0 ? "bg-olive text-ivory" : "border border-border text-muted-foreground"}`}
              >
                {t}
              </button>
            ),
