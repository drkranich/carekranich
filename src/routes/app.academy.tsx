...
const modules = [
  ...
  { t: "Olia platform mastery", lvl: "Platform", time: "2h", prog: 100, badges: 2 },
];
...
function Academy() {
  return (
    <>
      <PageHeader title="Caregiver academy" subtitle="Microlearning, simulations, certifications — continuous mastery." action={<Pill tone="gold">Level 4 · Specialist</Pill>} />
              ...
              <div key={m.t} className="rounded-2xl border border-border bg-cream/40 p-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{m.lvl} · {m.time} · {m.badges} badges</p>
                  </div>
            ...
            <ul className="mt-3 space-y-2 text-sm">
              {sims.map((s) => (
                <li key={s.t} className="rounded-xl border border-border/60 bg-cream/40 p-3">
                  ...
                  <p className="text-xs text-muted-foreground">{s.date} · <span className="text-moss">{s.grade}</span></p>
                </li>
              ))}
            </ul>
          </Card>
          ...
          <Card>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {["First Responder", "Dementia Care", "Family Whisperer", "Night Shift", "Cardiac", "100h Club"].map((b) => (
                <span key={b} className="rounded-full border border-gold/30 bg-gold/10 px-3 py-1 text-xs text-gold">★ {b}</span>
              ))}
            </div>
          </Card>
        </div>
