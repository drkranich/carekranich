...
type Score = {
  id: string;
  longevity_score: number | null; resilience_score: number | null; health_score: number | null;
  mobility_score: number | null; cognitive_score: number | null; emotional_score: number | null;
  social_score: number | null;
  risk_factors: unknown; protective_factors: unknown; methodology: unknown;
  computed_at: string;
};
...
type Assess = { vitality_score: number | null; emotional_stability_score: number | null; assessed_at: string };
...
function LongevityPage() {
  ...
  const { data: scores = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("longevity_scores").select("*")
        .eq("resident_id", residentId).order("computed_at", { ascending: true });
  ...
  const { data: insights = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("ai_insights").select("*")
        .eq("resident_id", residentId).eq("module", "longevity")
  ...
  const { data: obs = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase.from("twin_observations").select("domain,value_numeric,observed_at")
        .eq("resident_id", residentId).gte("observed_at", since.toISOString());
      return (data ?? []) as Obs[];
    },
  });
  ...
  const { data: assessments = [] } = useQuery({
    ...
    enabled: !!residentId,
    queryFn: async () => {
      const { data } = await supabase.from("cognitive_assessments")
        .select("vitality_score,emotional_stability_score,assessed_at")
        .eq("resident_id", residentId).order("assessed_at", { ascending: false }).limit(5);
  ...
  const compute = useMutation({
    mutationFn: async () => {
      ...
      const longevity = Math.round(
        (sub.health + sub.resilience + sub.mobility + sub.cognitive + sub.emotional + sub.social) / 6,
      );
      const risk: string[] = [];
  ...
  if (!residents.length) {
    return (
      <>
        <PageHeader title="Longevity Engine" subtitle="A premium longevity intelligence system for every resident." />
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={selected ? `${selected.preferred_name || selected.full_name} · ${scores.length} score snapshots` : ""}
        action={
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2 bg-gradient-olive text-ivory border-none" padded>
          <p className="text-xs uppercase tracking-widest text-ivory/70">Current longevity score</p>
          <div className="mt-4 flex items-end gap-6">
            <p className="font-display text-7xl">{latest?.longevity_score ?? "—"}</p>
            <div>
              <p className="text-xs text-ivory/70">Trajectory</p>
              <p className="font-display text-xl">{trajectoryLabel(scores)}</p>
            </div>
          </div>
          <p className="mt-4 max-w-md text-sm text-ivory/80">
            Computed from a 30-day rolling window over health, cognitive, emotional, mobility, and social signals captured by the Digital Twin.
          </p>
        </Card>

        <Card>
          <p className="text-xs uppercase tracking-widest text-muted-foreground">Resilience</p>
          <div className="mt-4">
            <Ring value={latest?.resilience_score ?? 0} label="Resilience index" sub="adaptive capacity" color="var(--wine)" size={150} />
          </div>
        </Card>
      </div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3 lg:grid-cols-6">
        {dims.map((d) => {
          ...
          return (
            <Card key={d.key}>
              <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">{d.label}</p>
              <p className="mt-2 font-display text-3xl text-olive">{v ?? "—"}</p>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-3">
        <Card>
          <p className="text-xs uppercase tracking-widest text-muted-foreground">Risk factors</p>
          {latest && Array.isArray(latest.risk_factors) && (latest.risk_factors as string[]).length ? (
            <ul className="mt-3 space-y-2 text-sm">
              {(latest.risk_factors as string[]).map((r, i) => (
                <li key={i} className="flex items-start gap-2 text-wine"><span className="mt-1 h-1.5 w-1.5 rounded-full bg-wine" />{r}</li>
              ))}
            </ul>
        ...
        <Card>
          <p className="text-xs uppercase tracking-widest text-muted-foreground">Protective factors</p>
          {latest && Array.isArray(latest.protective_factors) && (latest.protective_factors as string[]).length ? (
            <ul className="mt-3 space-y-2 text-sm">
              {(latest.protective_factors as string[]).map((r, i) => (
                <li key={i} className="flex items-start gap-2 text-moss"><span className="mt-1 h-1.5 w-1.5 rounded-full bg-moss" />{r}</li>
              ))}
            </ul>
          ) : <p className="mt-3 text-sm text-muted-foreground">Recompute to surface protective signals.</p>}
        </Card>

        <Card>
          <p className="text-xs uppercase tracking-widest text-muted-foreground">Areas for improvement</p>
            ...
            {latest ? dims
              ...
              .map((x) => (
                <li key={x.label} className="flex items-center justify-between">
                  <span>{x.label}</span><Pill tone="gold">{x.v}</Pill>
                </li>
      ...
      <div className="mt-8">
        <h2 className="mb-4 font-display text-xl text-foreground">AI recommendations</h2>
        {insights.length === 0 ? (
          <EmptyState title="No AI recommendations yet" hint="Recompute scores to enable AI synthesis, or wait for the insight system to populate." />
        ) : (
      ...
      <div className="mt-8">
        <h2 className="mb-4 font-display text-xl text-foreground">Longevity journey</h2>
        <Card>
          {scores.length === 0 ? (
            <p className="text-sm text-muted-foreground">No journey yet — milestones and score evolution will populate after the first recompute.</p>
          ) : (
            <ol className="space-y-4">
              {scores.slice().reverse().map((s, i) => (
                <li key={s.id} className="flex gap-4">
                  <div className="flex flex-col items-center">
                    <div className="flex h-8 w-8 items-center justify-center rounded-full bg-olive text-ivory font-display text-sm">{scores.length - i}</div>
                  ...
                  <div className="flex-1 rounded-2xl border border-border bg-cream/40 p-4">
                    ...
                    <p className="mt-1 font-display text-2xl text-olive">Longevity {s.longevity_score ?? "—"}</p>
...
function computeScores(obs: Obs[], assessments: Assess[]) {
  ...
  for (const o of obs) if (o.value_numeric != null) (byDomain[o.domain] ||= []).push(o.value_numeric);
  ...
  const emotional = assessments[0]?.emotional_stability_score
  ...
  return {
    health, mobility, social, emotional, cognitive, resilience,
