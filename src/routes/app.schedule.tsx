...
function Schedule() {
  ...
  const team = useQuery({
    ...
    queryFn: async () => {
      ...
      const nameOf = new Map((profiles ?? []).map((p: any) => [p.id, p.preferred_name || p.full_name || "Profissional"]));
      const seen = new Set<string>();
      return (roles ?? [])
        .filter((r: any) => (seen.has(r.user_id) ? false : (seen.add(r.user_id), true)))
        .map((r: any) => ({ id: r.user_id, name: nameOf.get(r.user_id) ?? "Profissional", role: r.role as AppRole }));
    },
  });
  ...
  const setWaitStatus = async (id: string, status: string, patientName?: string) => {
    ...
    if (status === "scheduled" && patientName) {
      ...
      toast.success("Preencha o horário do encaixe no formulário acima");
    }
    qc.invalidateQueries({ queryKey: ["schedule-waitlist", profile?.tenant_id] });
  };
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="ml-auto flex items-center gap-2">
            <input
              ...
              placeholder="Nova sala (ex.: Consultório 2)"
            ...
            <button onClick={addRoom} className="rounded-full border border-olive/30 bg-white/60 px-3 py-1.5 text-xs text-olive hover:bg-olive hover:text-ivory">
              <span className="inline-flex items-center gap-1"><DoorOpen className="h-3.5 w-3.5" /> Adicionar sala</span>
            </button>
          </div>
        </div>
      </Card>
      ...
      <Card className="mt-4">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-3 xl:grid-cols-6">
          ...
          <GlassSelect
            ...
            placeholder="Sala"
            options={[{ value: "", label: "Sem sala" }, ...(rooms.data ?? []).map((r: any) => ({ value: r.id, label: r.name }))]}
          />
          <GlassSelect
            value={form.kind}
          ...
          return (
            <Card key={professional.id}>
              <div className="flex items-center justify-between gap-2">
                ...
                <Pill tone="muted">{list.length} no dia</Pill>
              </div>
              <div className="mt-3 space-y-2">
                {list.length === 0 && <p className="py-4 text-center text-xs text-muted-foreground">Agenda livre neste dia.</p>}
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-1.5">
                        {appointment.kind === "encaixe" && <Pill tone="gold">encaixe</Pill>}
          ...
          <div className="mt-4 space-y-2">
            {(waitlist.data ?? []).map((item: any, index: number) => (
              <div key={item.id} className="flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-white/70 bg-white/50 px-4 py-2.5">
                ...
                <div className="flex items-center gap-1.5">
                  {item.status === "waiting" ? (
                    <button onClick={() => setWaitStatus(item.id, "called")} className="rounded-full border border-olive/30 bg-white/60 px-2.5 py-1 text-[11px] text-olive">Chamar</button>
                  ) : (
